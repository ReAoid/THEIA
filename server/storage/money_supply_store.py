"""
货币供应量数据专用存储 —— MoneySupplyStore

继承 BaseStore，提供 DataPoint ↔ dict 的序列化，
以及货币供应量特有的查询方法（按指标名、UUID、分组筛选）。

用法：
    from storage.money_supply_store import MoneySupplyStore
    from crawler.sources.money_supply_source import MoneySupplySource

    source = MoneySupplySource(period="200001-202606")
    data = source.flow()

    store = MoneySupplyStore()
    store.save(data)
...
class MoneySupplyStore(BaseStore[DataPoint]):
    """
    货币供应量数据专用存储。

    数据唯一键：(date, indicator, region) — 与 MoneySupplyManager 保持一致。
    序列化/反序列化直接复用 DataPoint.to_dict() 和构造方法。
    """

    store_name = "money_supply"
    ...
    def _point_key(self, item: DataPoint) -> tuple:
        """货币供应量数据的唯一键：(date, indicator, region)。"""
    ...
    def _deserialize(self, data: dict) -> DataPoint:
        ...
        return DataPoint(
            ...
            unit=data.get("unit", ""),
            source=data.get("source", "money_supply"),
    ...
    def get_by_group(self, group_name: str) -> list[DataPoint]:
        ...
        Returns:
            该分组下的全部 DataPoint
        """
        from crawler.sources.money_supply_source import MONEY_SUPPLY_GROUPS

        uuids = MONEY_SUPPLY_GROUPS.get(group_name)
        if not uuids:
            logger.warning(f"未知货币供应量分组: {group_name}")
            return []
