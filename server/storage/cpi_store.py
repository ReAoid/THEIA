...
用法：
    ...
    # 3. 按指标筛选
    overall = store.get_by_indicator("居民消费价格指数")
    core_cpi = store.get_by_group("核心CPI(8项)")
...
class CPIStore(BaseStore[DataPoint]):
    """
    CPI 数据专用存储。

    数据唯一键：(date, indicator, region)  — 与 CPIManager 保持一致。
    ...
    def _point_key(self, item: DataPoint) -> tuple:
        """CPI 数据的唯一键：(date, indicator, region)。"""
    ...
    def get_by_group(self, group_name: str) -> list[DataPoint]:
        ...
        Returns:
            该分组下的全部 DataPoint
        """
        from crawler.sources.cpi_source import CPI_GROUPS

        uuids = CPI_GROUPS.get(group_name)
        if not uuids:
            logger.warning(f"未知 CPI 分组: {group_name}")
            return []
