...
基于国家统计局新版 esData API，封装 PPI 特有的指标注册表、
分组查询等逻辑。
...
用法：
    engine = Engine()
    data = engine.run(PPISource(period="202605"), save=True)
...
# ── esData API 固定参数（PPI 专项页面）────────────────

DEFAULT_DA = [{"text": "全国", "value": "000000000000"}]

# ── CID 周期注册表 ────────────────────────────────────────
#
# PPI（工业生产者出厂价格指数）只有一套 CID，适用于全部历史年份。
...
CID_PERIODS = [
    {
        ...
        "indicator_ids": [
            "150633e52b9a470a9a9fd1b296dd6c5b",  # 工业生产者出厂价格指数 (总PPI)
            "47f8464961184392bc4f6a4b8e5b1cb5",  # 生产资料
            "649b27dbffb14466a3f204e16f3c74ed",  # 生活资料
        ],
    },
]
...
def get_period_config(year: int) -> dict:
    """根据年份查找对应的 CID 周期配置。PPI 始终返回唯一周期。"""
...
PPI_UUID_INDICATORS = {
    "150633e52b9a470a9a9fd1b296dd6c5b": "工业生产者出厂价格指数 (上年同月=100)",
    "47f8464961184392bc4f6a4b8e5b1cb5": "生产资料工业生产者出厂价格指数 (上年同月=100)",
    "649b27dbffb14466a3f204e16f3c74ed": "生活资料工业生产者出厂价格指数 (上年同月=100)",
}

# ── PPI 分组结构 ────────────────────────────────────
#
# 1. 总 PPI —— 工业生产者出厂价格指数（整体）
# 2. 两大分项 —— 生产资料 + 生活资料

PPI_GROUPS = {
    "总PPI": [
        "150633e52b9a470a9a9fd1b296dd6c5b",  # 工业生产者出厂价格指数
    ],
    "两大分项": [
        "47f8464961184392bc4f6a4b8e5b1cb5",  # 生产资料
        "649b27dbffb14466a3f204e16f3c74ed",  # 生活资料
...
# ═══════════════════════════════════════════════════════════
#  PPISource — PPI 数据源（新版 esData API）
# ═══════════════════════════════════════════════════════════

class PPISource(DataSource):
    """
    PPI 数据源（新版 esData API）。

    基于国家统计局新版 esData 接口，使用 JSON POST + UUID 指标 ID 查询 PPI 数据。
    """

    name = "ppi"

    def __init__(self, indicator_ids: list[str] | None = None,
                 period: str = "202306-202605",
        ...
        """
        Args:
            indicator_ids: PPI 指标 UUID 列表。默认为全部 3 个指标。
            period: 时间段。
                "202605"        → 单月
                "202306-202605" → 范围（默认）
                "2026"          → 全年
    ...
    def parse(self, raw: dict) -> list[DataPoint]:
          ...
          "data": [
            {
              ...
              "values": [
                {
                  "_id": "150633e52b9a470a9a9fd1b296dd6c5b",
                  "i_showname": "工业生产者出厂价格指数 (上年同月=100)",
                  "value": "103.9",
                  "du_name": "无",
                  "da_name": "国家",
                  ...
        ...
        for period_entry in data_list:
            ...
            for val_entry in values:
                ...
                registry_name = PPI_UUID_INDICATORS.get(uuid, "")
                ...
                if api_name:
                    ...
                    if (registry_name
                        ...
                        logger.warning(
                            ...
                            f"  注册表: {registry_name}\n"
                            f"  API返回: {api_name}\n"
                            f"  → 将使用 API 返回的名称，PPI_UUID_INDICATORS 可能需要更新"
                        )
                    ...
                    if uuid not in _uuid_warnings_emitted:
                        logger.warning(f"未知 UUID: {uuid}，PPI_UUID_INDICATORS 可能需要更新")
                        _uuid_warnings_emitted.add(uuid)

                unit = val_entry.get("du_name", "")
                # PPI 返回的 da_name 为 "国家"，统一使用
    ...
    def list_indicators(cls, group: str = None) -> list[dict]:
        ...
        """
        uuid_to_group = {}
        for g, uuids in PPI_GROUPS.items():
            for u in uuids:
                uuid_to_group[u] = g

        results = []
        for uuid, name in PPI_UUID_INDICATORS.items():
